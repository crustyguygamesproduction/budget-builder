...
async function buildServerCoachContext(req: Request, message: string) {
  ...
  return { server_built: true, totals: { ...totals, basis: "server_calculated_from_authenticated_user_rows" }, transaction_count: rows.length, recent_transactions: rows.slice(0, 30).map(toCoachTransaction), searchable_transactions: rows.slice(0, 350).map(toCoachTransaction), searchable_transaction_count: Math.min(rows.length, 350), searchable_transaction_note: rows.length > 350 ? `Most recent 350 transactions are included individually. Summaries use all ${rows.length} transactions.` : `All ${rows.length} transactions are included individually.`, query_focus: buildQueryFocus(rows, message), statement_intelligence: { date_range: { start: dates[0] || "", end: dates.at(-1) || "", total_transactions: rows.length }, totals, category_totals: groupTransactions(spending, (transaction) => transaction.category || "Spending", 20), merchant_totals: groupTransactions(spending, (transaction) => cleanLabel(transaction.merchant || transaction.description), 30), income_streams: groupTransactions(income, (transaction) => cleanLabel(transaction.merchant || transaction.description || "Income"), 20) }, debts: debts.slice(0, 6), investments: investments.slice(0, 6), goals: goals.slice(0, 6), money_understanding: snapshot?.ai_context || {}, bills_found: snapshot?.bill_streams || [], checks_waiting: snapshot?.checks || [], data_freshness: { latest_transaction_date: dates.at(-1) || null, latest_ai_interpretation: snapshot?.interpreted_at || null, latest_ai_transaction_date: snapshot?.latest_transaction_date || null }, recent_messages: aiMessages.slice(-6).map((msg: any) => ({ role: msg.role, content: msg.content })), safety_note: "Context was rebuilt inside the Edge Function from rows owned by the authenticated Supabase user. Client-supplied financial context is ignored for coach mode." };
}
...
function buildExtractionPrompt(kind: "debt" | "investment") {
  if (kind === "debt") {
    return `Extract debt setup information. Return JSON only with: name, lender, starting_balance, current_balance, minimum_payment, due_day, interest_rate, notes. Use null when unknown. Do not invent precise numbers.`;
  }
