  ...
  return (
    <div style={styles.app}>
      ...
      <main style={getMainStyle(screenWidth, page)}>
        ...
        {page === "calendar" && <CalendarPage transactions={transactions} />}
      ...
      <div style={styles.topBarText}>
        ...
        <p style={styles.topEmail}>
          {page === "coach"
            ...
            : page === "debts"
            ? "Debt tracking from your imported statements"
            : page === "investments"
...
function UploadPage({ accounts, onImportDone, screenWidth }) {
  ...
  function detectCategory(description, amount) {
    ...
    if (amount > 0 && text.includes("salary")) return "Income";
    if (amount > 0 && text.includes("wage")) return "Income";
    if (amount > 0 && text.includes("payroll")) return "Income";
    ...
    if (text.includes("transfer")) return "Internal Transfer";
    if (text.includes("savings")) return "Internal Transfer";
  ...
  function handleFile(event) {
    ...
    const matchingAccount = accounts.find(
      (account) =>
        account.name.toLowerCase() === guessedName.toLowerCase() ||
        account.nickname?.toLowerCase() === guessedName.toLowerCase()
    );

    if (matchingAccount) {
      setSelectedAccountId(matchingAccount.id);
    } else if (guessedName) {
      setNewAccountName(guessedName);
    }
  ...
  async function ensureAccount(userId) {
    ...
    const accountName =
      newAccountName.trim() || guessAccountName(fileName) || "Imported Account";
      ...
      .upsert(
        {
          ...
          institution: accountName.split(" ")[0],
  ...
  async function saveImport() {
    ...
    try {
      ...
      const { data: importRow, error: importError } = await supabase
        ...
        .insert({
          ...
          detected_account_name:
            newAccountName || accounts.find((a) => a.id === accountId)?.name,
          row_count: rows.length,
          import_summary: `Imported ${rows.length} rows from ${fileName}`,
        })
        .select()
        .single();
      ...
      alert("Imported. Any duplicate transactions were skipped.");
      setRows([]);
      setFileName("");
      setNewAccountName("");
  ...
  const previewDebtSignals = getDebtSignals(
    rows.map((row, index) => ({
      id: index,
      description: row.description,
      amount: row.amount,
  ...
  const previewInvestmentSignals = getInvestmentSignals(
    rows.map((row, index) => ({
      id: index,
      description: row.description,
      amount: row.amount,
  ...
  return (
    <>
      <Section title="Upload Statements">
        <p style={styles.sectionIntro}>
          Upload a CSV bank statement. I’ll detect income, bills, subscriptions,
          spending, accounts, debts, investing signals and other patterns for you.
        </p>
        ...
        {rows.length > 0 && (
          <>
            <label style={styles.label}>Which account is this?</label>
      ...
      {rows.length > 0 && (
        <>
          <div style={getGridStyle(screenWidth)}>
            <MiniCard title="Income" value={`£${preview.income.toFixed(2)}`} />
            <MiniCard title="Spending" value={`£${preview.spending.toFixed(2)}`} />
            <MiniCard title="Debt Signals" value={`${previewDebtSignals.length}`} />
            <MiniCard title="Investing Signals" value={`${previewInvestmentSignals.length}`} />
          </div>

          <Section title="AI Import Summary">
            <p style={styles.sectionIntro}>
              I found {rows.length} transactions in {fileName}. Categories are
              pre-guessed, duplicates will be skipped, and I’m already scanning
              for debt and investing activity too.
            </p>
          </Section>

          <Section title="Detected Transactions">
            ...
            {rows.length > 12 && (
              <p style={styles.smallMuted}>Showing first 12 rows.</p>
            )}
          </Section>

          <button style={styles.primaryBtn} onClick={saveImport} disabled={saving}>
            {saving ? "Importing..." : "Import Into My Hub"}
          </button>
        </>
      )}
    </>
  ...
  return (
    <>
      <Section title="Debt Tracker">
        <p style={styles.sectionIntro}>
          This now handles AI-detected payment streams and plain-English setup.
          It also checks whether each debt looks paid this month, due soon, or worth checking.
        </p>
      </Section>
  ...
  return (
    <>
      <Section title="Investment Tracker">
        <p style={styles.sectionIntro}>
          This now handles AI-detected broker funding and plain-English setup.
          It also checks whether contributions are still happening this month.
        </p>
      </Section>
...
function CalendarPage({ transactions }) {
  const moneyEvents = transactions
    .filter((t) => t.is_bill || t.is_subscription || t.is_income)
    .slice(0, 20);
  ...
  return (
    <Section title="Money Calendar">
      {moneyEvents.length === 0 ? (
        <p style={styles.emptyText}>
          Upload statements to build your payment calendar.
        </p>
      ) : (
        moneyEvents.map((t) => (
          <Row
  ...
  return (
    <>
      ...
      <Section title="Product Direction">
        ...
        <Row name="Debt records + AI setup" value="Live" />
        <Row name="Investment records + AI setup" value="Live" />
        <Row name="Monthly smart tracking" value="Live" />
